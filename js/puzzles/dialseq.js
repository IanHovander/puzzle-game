  ...
  function build(container, cfg, api) {
    return new Promise((resolve) => {
      ...
      function render() {
        ...
        cfg.glyphs.forEach(g => palette.appendChild(UI.el('button', { class: 'glyph', html: `<svg viewBox="-20 -20 40 40">${g.svg}</svg><span class="gi">${UI.esc(g.label || g.id)}</span>`, onclick: () => { if (sel == null) { Audio.sfx('wrong'); return; } if (cfg.maxTurns && turns.length >= cfg.maxTurns) { Audio.sfx('wrong'); status.textContent = 'The door counts no more than ' + cfg.maxTurns + ' turns.'; return; } turns.push({ dial: cfg.dials[sel].id, glyph: g.id }); Audio.sfx('click'); sel = null; render(); } })));
        UI.clear(turnsEl);
