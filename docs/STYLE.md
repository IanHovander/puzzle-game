...
still turns out to be broken -- sixteen patterns, each taken from a real defect in this game, each one
invisible from inside the chapter it lives in. Two of them are about how the checking itself goes
wrong. A chapter is not done because it passes the checklist; it is done when somebody has tried the
sixteen and failed to get in.
