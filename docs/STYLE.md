...
- **R1.1 — Ship gate.** `node tools/scan-fit.js chN --w 1280 --h 720` reports **`overflowing: none`**, with **at most 2 scenes shrunk** and **none below 15 px**.
...
- **R1.4** A puzzle brief (`text` on a `type:'puzzle'` scene, which renders at 18 px in a 44 vh box) is **≤ 90 words and ≤ 8 lines, each line ≤ 20 words**. `ch0_lamp` = 48 words / 4 lines. `ch1_vote` = 86 words / 8 lines, and it is the scene that shrinks — treat 8 lines as the ceiling, not the target.
